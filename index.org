...
$areaFunctionMap = [
  ...
  'Triangle' => function($dog) {
                  return false;
                }
];

$area = $shape->callYourFunction($areaFunctionMap);
#+END_SRC
...
interface ShapeFunctions {
  function functionForCircle($dog);
  function functionForSquare($cat);
  function functionForTriangle($goldfish);
}
#+END_SRC
...
#+ATTR_REVEAL: :frag appear
#+BEGIN_SRC php
class HasFurFunctions implements ShapeFunctions {
  function functionForCircle($dog) { return true; }
  function functionForSquare($cat) { return true; }
  function functionForTriangle($goldfish) { return false; }
}
#+END_SRC
...
** Another Solution (Continued)
Finally, use the ~HasFur~ ~ShapeFunctions~:
#+BEGIN_SRC php
$area = $shape->callYourFunction(new HasFurFunctions());
#+END_SRC
